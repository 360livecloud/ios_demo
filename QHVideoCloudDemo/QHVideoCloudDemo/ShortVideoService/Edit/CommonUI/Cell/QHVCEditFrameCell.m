...
- (void)setImageTypeCircle:(BOOL)isCircle
{
    if (isCircle)
    {
        [_frameImageView.layer setCornerRadius:CGRectGetHeight(_frameImageView.frame)/2.0];
        [_frameImageView.layer setMasksToBounds:YES];
    }
    else
    {
        [_frameImageView.layer setCornerRadius:0];
        [_frameImageView.layer setMasksToBounds:NO];
    }
}
